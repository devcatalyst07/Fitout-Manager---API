  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      if (!brandFilter || !["all", "specific"].includes(brandFilter)) {
        return res
          .status(400)
          .json({ message: "Valid brand filter is required" });
      }
      ...
      if (brandFilter === "specific") {
        if (!brandId) {
          return res
            .status(400)
            .json({
              message: "Brand ID is required for specific brand filter",
            });
        }
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      if (existingWorkflow) {
        return res
          .status(400)
          .json({ message: "Workflow name already exists in this scope" });
      }
    ...
    try {
      ...
      if (name && name !== workflow.name) {
        ...
        if (duplicate) {
          return res
            .status(400)
            .json({ message: "Workflow name already exists" });
        }
      }
...
router.get(
  "/:scopeId/workflows/:workflowId/phases",
  authMiddleware,
  adminOnly,
  async (req: express.Request, res: express.Response) => {
    try {
...
// UPDATE phase
router.put(
  "/:scopeId/workflows/:workflowId/phases/:phaseId",
  authMiddleware,
  adminOnly,
  async (req: express.Request, res: express.Response) => {
    try {
...
router.get(
  "/:scopeId/workflows/:workflowId/phases/:phaseId/tasks",
  authMiddleware,
  adminOnly,
  async (req: express.Request, res: express.Response) => {
    try {
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      if (taskOrder === undefined) {
        const maxTask = await Task.findOne({ phaseId, isTemplate: true }).sort({
          order: -1,
        });
...
// UPDATE task
router.put(
  "/:scopeId/workflows/:workflowId/phases/:phaseId/tasks/:taskId",
  authMiddleware,
  adminOnly,
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      const templateData = [
        {
          ...
          "Task Description":
            "Conduct comprehensive site survey and document existing conditions",
          "Task Type": "Task",
          Priority: "High",
          "Predecessor IDs": "",
          "Dependency Type": "",
...
router.get(
  "/:scopeId/workflows/:workflowId/export",
  authMiddleware,
  adminOnly,
  async (req: express.Request, res: express.Response) => {
    try {
