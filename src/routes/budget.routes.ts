...
router.get(
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to fetch budget", error: error.message });
    }
  },
);
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      if (!description || !vendor || !category) {
        return res
          .status(400)
          .json({ message: "Description, vendor, and category are required" });
      }
      ...
      const populatedItem = await BudgetItem.findById(
        newBudgetItem._id,
      ).populate("createdBy", "name email");
